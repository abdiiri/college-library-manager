...
const navItems = [
  { to: "/dashboard", label: "Dashboard", icon: LayoutDashboard, roles: ["admin", "librarian", "student"] },
...
export default function AppLayout({ children }: { children: ReactNode }) {
  const { profile, signOut } = useLibrary();
  const location = useLocation();

  if (!profile) return null;

  const filtered = navItems.filter((n) => n.roles.includes(profile.role));
      ...
      <aside className="w-64 bg-sidebar text-sidebar-foreground flex flex-col shrink-0">
        ...
        <div className="px-3 py-4 border-t border-sidebar-border">
          <div className="px-3 py-2 mb-2">
            <p className="text-sm font-medium text-sidebar-accent-foreground">{profile.full_name || "User"}</p>

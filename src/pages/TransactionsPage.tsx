...
export default function TransactionsPage() {
  ...
  const activeTx = transactions.filter((t) => !t.return_date);
  ...
  const handleIssue = async () => {
    ...
    const success = await issueBook(selectedMember, selectedBook, dueDate);
    if (success) {
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select value={selectedMember} onValueChange={setSelectedMember}>
                ...
                <SelectContent>
                  {activeMembers.map((m) => (
                    <SelectItem key={m.id} value={m.id}>{m.name} ({m.member_id})</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div>
              ...
              <Select value={selectedBook} onValueChange={setSelectedBook}>
                <SelectTrigger className="mt-1"><SelectValue placeholder="Choose book" /></SelectTrigger>
                <SelectContent>
                ...
                {activeTx.map((tx) => {
                  ...
                  return (
                    <tr key={tx.id}>
                      ...
                      <td>
                        <Button size="sm" variant="outline" onClick={() => handleReturn(tx.id)}>Return</Button>
                      </td>
                    </tr>
                  );
                })}
                {activeTx.length === 0 && (
                  <tr><td colSpan={6} className="text-center text-muted-foreground py-8">No active borrowings</td></tr>
                )}
              </tbody>
            </table>
          </div>
